...
import './index.scss';
import { getCartData, postCartDelete, postCartCheck, postCartUpdate } from '../../http/getData';
import isCheck from '../../assets/images/isCheck.png';
import noCheck from '../../assets/images/noCheck.png';
import noGoods from '../../assets/images/noGoods.png';
...
class Cart extends Component {
    ...
    async fetchCartData() {
        ...
        try {
            startLoading();
            const { errno, errmsg, cartList, cartTotal } = await getCartData({});
            console.log( errno, errmsg, cartList, cartTotal)
            endLoading();
            if (errno === 401) {
                Toast.fail(errmsg, .5, () => {
    ...
    async checkCartGoods(item) {
        ...
        try {
            startLoading()
            const { errno, errmsg, data } = await postCartCheck({ isChecked: item.checked === 1 ? 0 : 1, productIds: item.product_id })
            if (errno === 401) {
                Toast.fail(errmsg, .5, () => {
                    this.props.actions.loginFailure()
                })
    ...
    async addOrDelete(item, type, cartList) {
        ...
        if (number + type > 0) {
            ...
            try {
                startLoading()
                const { errno, errmsg, data } = await postCartUpdate({ goodsId: goods_id, id, number: number + type, productId: product_id })
                endLoading()
                if (errno === 0) {
    ...
    async checkAllCartGoods(cartList) {
        ...
        try {
            startLoading()
            const { errno, errmsg, data } = await postCartCheck({ isChecked: bol ? 0 : 1, productIds: ids.join(',') })
            if (errno === 0) {
                const { cartList, cartTotal: { checkedGoodsAmount, goodsCount, checkedGoodsCount } } = data
                cartList.forEach((item) => {
                    item.isHasEdit = false
    ...
    async deleteHasEdit(cartLists) {
        ...
        try {
            ...
            const { errno, errmsg, data } = await postCartDelete({ productIds: productIds.join(',') })
            if (errno === 0) {
                const { cartList, cartTotal: { checkedGoodsAmount, goodsCount, checkedGoodsCount } } = data
                cartList.forEach((item) => {
                    item.isHasEdit = false
