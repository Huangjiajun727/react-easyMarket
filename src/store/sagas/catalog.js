import { getCatalogInitData, getCatalogMsg } from '../../http/getData'
...
export function* catalogInit(action) {
  try {
    const { categoryList, currentCategory } = yield call(getCatalogInitData, {});
    // catelogId非NULL,则刷新当前catelogId分类数据
    if (action.parmas.catelogId) {
      const { currentCategory } = yield call(getCatalogMsg, { id: action.parmas.catelogId });
      yield put({
...
export function* catalogMsg(action) {
  try {
    const data = yield call(getCatalogMsg, action.parmas);
